					...
					if ( t.obj.section != 'homeBtn' ){
						...
						if ( t.obj.crsSelected != "" ){
							...
							if (t.obj.section == "ospAppBtn"){
								...
								$('#' + t.id + 'dlAccord').accordion( "option", "active", t.obj.activeAcIndex )	
								$('#' + t.id + 'dlAccord1').accordion( "option", "active", t.obj.activeAc1Index )	
								// Open OPS element summaries
								$.each(t.obj.ospElementsVis, lang.hitch(t,function(i, v){
									$('#' + t.id + 'elementsWrapper .firstIndent').each(lang.hitch(t,function(j, w){
										if (v == j){
